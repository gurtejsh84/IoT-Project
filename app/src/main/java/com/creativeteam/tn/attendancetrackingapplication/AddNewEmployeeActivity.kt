    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnSave.setOnClickListener {
            if(TextUtils.isEmpty(edName.text)){
                ...
                val query = ParseQuery.getQuery<ParseObject>("Employee")
                ...
                query.countInBackground{count, e ->
                    if(count > 0){
                        ...
                        file.saveInBackground(SaveCallback { e ->
                            if(e==null){
                                val employee = ParseObject("Employee")
                                employee.put("name", edName.text.toString())
                                employee.put("ssn", edSSN.text.toString())
                                employee.put("picture", file)
                                employee.saveInBackground { err ->
                                    if(err == null){
                                        progressBar.visibility = View.GONE
                                        Toast.makeText(this, "The employee is successfully saved!", Toast.LENGTH_LONG).show()
                                    }else{
                                        Log.d("AddNewEmployeeActivity", "Error : "+err.message)
                                    }
                                }
                            }else{
                                Log.d("AddNewEmployeeActivity", "Error : "+e.message)
                            }
                        })
                    }
                }
